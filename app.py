...
# CONFIGURATION
# --------------------------------------------------
st.set_page_config(page_title="DE Copilot", page_icon="🚀", layout="wide")
...
* ❄️ Snowflake DDL
* 🔁 Snowflake SQL (Inference Pipeline)
* 📖 Data Dictionary
...
def generate_ai_analysis(df):
    ...
    try:
        response = client.chat.completions.create(
            model="gpt-4o-mini",  # Adjusted to valid production model identifier
            messages=[{"role": "user", "content": prompt}],
            temperature=0.2,
        )
        return response.choices[0].message.content
    except Exception as ai_err:
        return f"Error connecting to OpenAI service: {str(ai_err)}"
...
COLUMN_ALIASES = {
    ...
    "LENGTH": ["LENGTH", "TARGET_LENGTH", "TARGET_COLUMN_LENGTH"],
    "PRECISION": ["PRECISION", "TARGET_PRECISION"],
    "SCALE": ["SCALE", "TARGET_SCALE"],
    "NULLABLE": ["NULLABLE", "TARGET_IS_NULLABLE", "IS_NULLABLE", "TARGET_NULLABLE"],
    "BUSINESS_DEFINITION": [
        "BUSINESS_DEFINITION",
        "SOURCE_DESCRIPTION",
        "TARGET_DESCRIPTION",
...
if uploaded_file:
    try:
        ...
        st.caption(
            f"Rows uploaded: {len(df):,} | Columns detected: {len(df.columns):,}"
        )
        ...
        for _, row in df.iterrows():
            ...
            data_type = build_snowflake_type(
                row, datatype_col, length_col, precision_col, scale_col
            )
            ...
            ddl_columns.append(f"    {target_column} {data_type} {nullable}".rstrip())
        ...
        for _, row in df.iterrows():
            ...
            if transformation and transformation.upper() not in ["N/A", "NA", "NONE"]:
                ...
                sql_lines.append(f"    {source_column} AS {target_column}")
        ...
        with tab1:
            ...
            st.download_button(
                "Download DDL", ddl, file_name="snowflake_ddl.sql", mime="text/plain"
            )

        with tab2:
            st.code(sql, language="sql")
            st.download_button(
                "Download SQL", sql, file_name="snowflake_sql.sql", mime="text/plain"
            )
        ...
        with tab6:
            ...
            st.info(
                "AI analysis uses the first 100 rows only to control cost and improve speed."
            )
