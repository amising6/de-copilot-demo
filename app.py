...
This engine uses a **Deterministic Heuristics Engine + Budget-Capped LLM Semantic Mapping Pipeline** to dynamically parse *any* vendor layout without structural hardcoding.
""")

# --------------------------------------------------
# PHASE 1: DETERMINISTIC HEURISTIC PARSING
# --------------------------------------------------
...
def guess_columns_heuristically(uploaded_headers):
    ...
    targets = {
        "SOURCE_TABLE": ["SRC_TBL", "SRC_TABLE", "SOURCE_TABLE", "SOURCE_ENTITY", "SOURCE_PHYSICAL_NAME"],
        "SOURCE_COLUMN": ["SRC_COL", "SRC_COLUMN", "SOURCE_COLUMN", "SOURCE_FIELD"],
        "TARGET_TABLE": ["TGT_TBL", "TARGET_TABLE", "TARGET_ENTITY", "DESTINATION_TABLE"],
        "TARGET_COLUMN": ["TGT_COL", "TARGET_COLUMN", "TARGET_FIELD", "DESTINATION_COLUMN"],
        "DATA_TYPE": ["DATA_TYPE", "DATATYPE", "TARGET_DATA_TYPE", "TGT_DATA_TYPE", "TYPE"],
        "LENGTH": ["LENGTH", "SIZE", "TARGET_LENGTH", "MAX_LEN"],
        "PRECISION": ["PRECISION", "NUMERIC_PRECISION", "TARGET_PRECISION"],
        "SCALE": ["SCALE", "NUMERIC_SCALE", "TARGET_SCALE"],
        "NULLABLE": ["NULLABLE", "IS_NULLABLE", "TARGET_IS_NULLABLE", "NULL_INDICATOR"],
        "TRANSFORMATION_RULE": ["TRANSFORMATION", "LOGIC", "TRANSFORMATION_LOGIC", "BUSINESS_RULE"],
        "BUSINESS_DEFINITION": ["DESCRIPTION", "DEFINITION", "BUSINESS_DEFINITION", "SOURCE_DESCRIPTION"],
        "DQ_RULE": ["DQ", "DATA_QUALITY", "DQ_RULE", "VALIDATION_RULE"]
    }
    ...
    for key, patterns in targets.items():
        ...
        if not matched:
            for norm_h, original in header_map.items():
                if any(p in norm_h for p in patterns if len(p) > 3):
                    guessed_mapping[key] = original
                    break
            else:
                guessed_mapping[key] = None
...
def guess_columns_with_ai(uploaded_headers):
    ...
    try:
        response = client.chat.completions.create(
            ...
            max_tokens=80  # Strict cost ceiling protecting token budget
        )
